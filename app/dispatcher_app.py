...
try:
    ...
    try:
        ...
        st.error("CRITICAL: Could not find 'utils/translations.py'.")
        st.stop()
...
def render_app():
    ...
    if menu == "Dashboard":
        ...
        if role in ['dispatch', 'admin']:
            with st.expander(translate('post', lang)):
                ...
                if st.button("Post Job"):
                    sb.table("jobs").insert({"title": title_in, "revenue": rev_in, "user_id": user.id, "status": "pending"}).execute()
                    st.success("Job live!"); st.rerun()
        ...
        try:
            ...
            for j in jobs_res:
                with st.container(border=True):
                    ...
                    col1.write(f"**{boost_tag}{j['title']}** (${j['revenue']})")
                    col2.write(f"Status: `{j['status']}`")
                    if role == 'driver' and j['status'] == 'pending':
                        if col3.button(translate('claim', lang), key=f"c_{j['id']}"):
                            ...
                            st.rerun()
                    elif j['status'] == 'in_progress' and j.get('driver_id') == user.id:
                        up_file = st.file_uploader("Upload Proof", type=['png', 'jpg', 'pdf'], key=f"f_{j['id']}")
                        if up_file: handle_upload(j, up_file, user.id)
        except Exception as e: st.error(f"Error loading jobs: {e}")

    elif menu == "Earnings":
        st.header(translate('earnings', lang))
        e_data = sb.table("jobs").select("revenue").eq("driver_id", user.id).eq("status", "completed").execute().data
        total = sum(d['revenue'] for d in e_data) if e_data else 0
        st.metric("Total Payout", f"${total:,.2f}")

    elif menu == "Premium":
        st.subheader("Account Upgrades")
        if is_premium: st.success("Premium status active! 👑")
        else:
        ...
        with tab2: translation_editor()
        with tab3: run_diagnostic()

    if st.sidebar.button("Sign Out"):
        st.session_state.clear()
        st.rerun()

# ... [auth_page and handle_upload remain unchanged from your working version] ...
